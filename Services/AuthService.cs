    ...
    public class AuthService : IAuthService {
        ...
        public async Task<LoginRes> LoginAsync(SignInReq? signInReq, ClaimsPrincipal? currentUser) {
            ...
            if (signInReq.VerificationId.HasValue) {
                ...
                // prefer ClaimTypes.NameIdentifier (works with ASP.NET mapping); fallback to "sub"
                var userIdClaim = currentUser?.FindFirstValue(ClaimTypes.NameIdentifier) ?? currentUser?.FindFirstValue(JwtRegisteredClaimNames.Sub);
        ...
        public async Task<OtpRes> SendOtpAsync(SendOtpReq sendOtpReq, ClaimsPrincipal? currentUser) {
             ...
             * შენიშვნა: არ უნდა მოხდეს ამისა შენახვა არც ცხრილში და არც ლოგებში.
            */
